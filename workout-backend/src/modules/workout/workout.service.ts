...
export class WorkoutService {
  ...
  private parseScheduledDays(rawValue: unknown): string[] {
    ...
    if (Array.isArray(rawValue)) {
      return rawValue.filter(day => typeof day === "string" && day.trim().length > 0);
    }
    if (typeof rawValue === "object") {
      const bag = rawValue as { scheduledDays?: unknown; days?: unknown };
      const nestedValue = bag.scheduledDays ?? bag.days;
      if (Array.isArray(nestedValue)) {
        return nestedValue.filter(day => typeof day === "string" && day.trim().length > 0);
      }
    }
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        return parsed.filter(day => typeof day === "string" && day.trim().length > 0);
      }
  ...
  private async hydrateWorkoutExercises(workoutId: number) {
    ...
    const exercises = await this.exerciseRepository.find({ where: { id: In(exerciseIds) } });
  ...
  async createWorkout(createWorkoutDto: any, authUUID: string) {
    try {
      ...
      const hydratedExercises = await this.hydrateWorkoutExercises(savedWorkout.id);
      return { ...savedWorkout, exercises: hydratedExercises };
    } catch (error) {
      console.error("Failed to create workout:", error);
      throw new HttpException("Failed to create workout", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  async updateWorkout(workoutId: number, updateWorkoutDto: any, authUUID: string) {
    try {
      ...
      workout.totalDuration = updateWorkoutDto.totalDuration ?? workout.totalDuration;
      ...
      throw new HttpException("Failed to update workout", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async listWorkouts(authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to list workouts", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getWorkout(workoutId: number, authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to fetch workout", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async deleteWorkout(workoutId: number, authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to delete workout", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async createExercise(createExerciseDto: any, authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to create exercise", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async listExercises() {
    try {
      ...
      throw new HttpException("Failed to list exercises", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getExercise(exerciseId: number) {
    try {
      ...
      throw new HttpException("Failed to fetch exercise", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  async listFavoriteExercises(authUUID: string) {
    try {
      const favorites = await this.exerciseFavoriteRepository.find({ where: { userUUID: authUUID } });
      ...
      throw new HttpException("Failed to list favorite exercises", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async toggleExerciseFavorite(exerciseId: number, authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to toggle exercise favorite", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async createRoutine(createRoutineDto: any, authUUID: string) {
    try {
      ...
      routine.scheduledDays = this.serializeScheduledDays(createRoutineDto.scheduledDays) ?? null;
      ...
      return { ...savedRoutine, exercises, scheduledDays: this.parseScheduledDays(savedRoutine.scheduledDays) };
    } catch (error) {
      console.error("Failed to create routine:", error);
      throw new HttpException("Failed to create routine", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async listRoutines(authUUID: string) {
    try {
      ...
      const favorites = await this.routineFavoriteRepository.find({ where: { userUUID: authUUID } });
      ...
      throw new HttpException("Failed to list routines", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getRoutine(routineId: number, authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to fetch routine", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  async updateRoutine(routineId: number, updateRoutineDto: any, authUUID: string) {
    try {
      const routine = await this.routineRepository.findOne({ where: { id: routineId } });
      if (!routine) {
        throw new HttpException("Routine not found", HttpStatus.NOT_FOUND);
      ...
      routine.scheduledDays =
        this.serializeScheduledDays(updateRoutineDto.scheduledDays) ?? routine.scheduledDays;
      ...
      routine.estimatedDuration = updateRoutineDto.estimatedDuration ?? routine.estimatedDuration;
      routine.estimatedCalories = updateRoutineDto.estimatedCalories ?? routine.estimatedCalories;
      ...
      throw new HttpException("Failed to update routine", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async deleteRoutine(routineId: number, authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to delete routine", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async listFavoriteRoutines(authUUID: string) {
    try {
      ...
      const routines = await this.routineRepository.find({ where: { id: In(routineIds) } });
      ...
      throw new HttpException("Failed to list favorite routines", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async toggleRoutineFavorite(routineId: number, authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to toggle routine favorite", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async createProgress(createProgressDto: any, authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to create progress", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async listProgress(authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to list progress", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  async getProgressMetrics(authUUID: string) {
    try {
      const workouts = await this.workoutRepository.find({ where: { userUUID: authUUID } });
      ...
      throw new HttpException("Failed to fetch progress metrics", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  private buildWeeklyData(workouts: WorkoutEntity[]) {
    ...
    const totals = labels.map(label => ({ day: label, workouts: 0, calories: 0 }));
    ...
    workouts.forEach(workout => {
      ...
      const offset = Math.floor((workoutDate.getTime() - startOfWeek.getTime()) / 86400000);
  ...
  private buildMonthlyData(workouts: WorkoutEntity[]) {
    ...
    for (let i = 3; i >= 0; i--) {
      ...
      months.push({ label, year: date.getFullYear(), monthIndex: date.getMonth() });
    }

    const totals = months.map(item => ({ month: item.label, workouts: 0, calories: 0 }));
    ...
    workouts.forEach(workout => {
      ...
      months.forEach((item, index) => {
        if (workoutDate.getFullYear() === item.year && workoutDate.getMonth() === item.monthIndex) {
  ...
  async getDashboardSummary(authUUID: string) {
    try {
      ...
      const totalDuration = workouts.reduce((sum, workout) => sum + Number(workout.totalDuration || 0), 0);
      const completedWorkouts = workouts.filter(workout => workout.isCompleted).length;
      ...
      const weeklyWorkouts = workouts.filter(workout => new Date(workout.date) >= weekAgo).length;
      const monthlyWorkouts = workouts.filter(workout => new Date(workout.date) >= monthAgo).length;
      ...
      throw new HttpException("Failed to fetch dashboard summary", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  private async getImprovementAreas(workouts: WorkoutEntity[]) {
    ...
    const exerciseIds = Array.from(new Set(workoutExercises.map(ex => ex.exerciseId)));
    const exercises = await this.exerciseRepository.find({ where: { id: In(exerciseIds) } });
    const typeMap = new Map(exercises.map(ex => [ex.id, (ex.type || "").toLowerCase()]));
  ...
  async getRecentWorkouts(authUUID: string) {
    try {
      ...
      throw new HttpException("Failed to fetch recent workouts", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getWorkoutTemplates() {
    try {
      ...
      throw new HttpException("Failed to fetch workout templates", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  async getReminder(authUUID: string) {
    try {
      const reminder = await this.workoutReminderRepository.findOne({ where: { userUUID: authUUID } });
      if (!reminder) {
        return {
          enabled: false,
          timeOfDay: null,
      ...
      throw new HttpException("Failed to fetch reminder", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  async setReminder(authUUID: string, payload: { enabled?: boolean; timeOfDay?: string; days?: string[] }) {
    try {
      const existing = await this.workoutReminderRepository.findOne({ where: { userUUID: authUUID } });
      ...
      reminder.days = this.serializeScheduledDays(payload.days) ?? reminder.days ?? null;
      ...
      throw new HttpException("Failed to update reminder", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
